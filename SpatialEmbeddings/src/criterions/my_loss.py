...
class SpatialEmbLoss(nn.Module):
    ...
    def __init__(self, to_center=True, n_sigma=1, foreground_weight=1, foreground_weight_sem=1, ):
        super().__init__()
        ...
        self.foreground_weight_sem = torch.FloatTensor(foreground_weight_sem)
        self.ce_criteria = nn.CrossEntropyLoss(weight=self.foreground_weight_sem, ignore_index=255)
    ...
    def forward(self, output_sem, output_ins, instances, labels, semantic_label, w_inst=1, w_var=10, w_seed=1,
                iou=False, iou_meter=None):

        loss = 0
        ce_loss = self.ce_criteria(output_sem, semantic_label)

        batch_size, height, width = output_ins.size(
            0), output_ins.size(2), output_ins.size(3)
        ...
        for b in range(0, batch_size):
            ...
            spatial_emb = torch.tanh(output_ins[b, 0:2]) + xym_s  # 2 x h x w
            sigma = output_ins[b, 2:2 + self.n_sigma]  # n_sigma x h x w
            seed_maps = torch.sigmoid(
                output_ins[b, 2 + self.n_sigma:2 + self.n_sigma + 8])  # 1 x h x w
            ...
            for i in range(8):
                ...
                instance_ids = instance[label == i + 1].unique()
                ...
                for id in instance_ids:
                    ...
                    # seed loss
                    seed_loss += self.foreground_weight[i] * torch.sum(
            ...
            ins_loss += w_inst * instance_loss + w_var * var_loss + w_seed * seed_loss

        ins_loss = ins_loss / (b + 1)
        loss += ins_loss + ce_loss

        return loss, ce_loss, ins_loss
