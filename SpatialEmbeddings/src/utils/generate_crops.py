...
def process(tup):
    ...
    image_path = os.path.join(IMAGE_DIR, 'crops_' + str(OBJ_ID), image_path)
    instance_path = os.path.splitext(os.path.relpath(inst, os.path.join(INSTANCE_DIR, 'train')))[0]
    instance_path = os.path.join(INSTANCE_DIR, 'crops_' + str(OBJ_ID), instance_path)
    semantic_path = os.path.splitext(os.path.relpath(sem, os.path.join(INSTANCE_DIR, 'train')))[0]
    semantic_path = os.path.join(INSTANCE_DIR, 'crops_' + str(OBJ_ID), semantic_path)
    ...
    for j, id in enumerate(ids):
        ...
        semantics_crop = semantics.crop((jj, ii, jj + CROP_SIZE, ii + CROP_SIZE))

        im_crop.save(image_path + "_{:03d}.png".format(j))
        instance_crop.save(instance_path + "_{:03d}.png".format(j))
        semantics_crop.save(semantic_path + "_{:03d}.png".format(j))
...
if __name__ == '__main__':
    ...
    OBJ_IDs = [24, 25, 26, 27, 28, 31, 32, 33]
    CROP_SIZE = 512
    ...
    instances.sort()
    semantics = glob.glob(os.path.join(INSTANCE_DIR, 'train', '*/*_gtFine_labelTrainIds.png'))
    semantics.sort()

    for OBJ_ID in OBJ_IDs:
        with Pool(8) as p:
            r = list(tqdm(p.imap(process, zip(images, instances, semantics)), total=len(images)))
