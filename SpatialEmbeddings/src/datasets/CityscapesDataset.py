...
class CityscapesDataset(Dataset):
    ...
    def __init__(self, root_dir='./', type="train", class_id=26, size=None, transform=None):
        ...
        semantic_list = glob.glob(os.path.join(root_dir, 'gtFine/{}/'.format(type), '*/*_gtFine_labelTrainIds*.png'))
        semantic_list.sort()
    ...
    def __getitem__(self, index):
        ...
        # load semantic label
        semantic_label = Image.open(self.semantic_list[index])
        sample['semantic_label'] = self.ignore_index(semantic_label)
    ...
    def ignore_index(self, semtantic_label):
        pic = np.array(semtantic_label, copy=False)
        semantic_map = np.zeros(
            (pic.shape[0], pic.shape[1]), dtype=np.uint8)
        uni = list(np.unique(pic.flatten()))
        list_to_ignore = [11, 12, 13, 14, 15, 16, 17, 18]
        for i in uni:
            if i in list_to_ignore:
                semantic_map[pic == i] = 11
            else:
                semantic_map[pic == i] = pic[pic == i]
